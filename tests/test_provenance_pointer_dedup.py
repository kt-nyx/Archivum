...
from pipeline.generate.draft.pages.assembly import _pointers_for_source_ids


def test_pointers_for_source_ids_dedupes_repeated_snippet() -> None:
    ...
    pointers = _pointers_for_source_ids(pool, ["src-a"], revision_map)
    assert len(pointers) == 1
    assert len({p["excerpt_hash"] for p in pointers}) == 1


def test_pointers_for_source_ids_keeps_distinct_snippets() -> None:
    pool = [
        {"source_id": "src-a", "snippet": "First fact.", "content_role": "history"},
        {"source_id": "src-a", "snippet": "Second fact.", "content_role": "history"},
    ]
    pointers = _pointers_for_source_ids(pool, ["src-a"], {"src-a": "mw:1"})
