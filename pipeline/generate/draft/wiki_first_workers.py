...
Option F instance cast selection (S2+), intended S3 wire-up::

    raw = collect_character_pool(...)
    pool = prefilter_character_pool(raw, instance_name=...)
    floor = must_include_key_character_names(boss_pool_items=..., pool=pool, instance_name=...)
    remaining = INSTANCE_MAX_KEY_CHARACTERS - len(floor)
    if remaining > 0 and pool:
        prompt_pool = cap_pool_for_llm_prompt(pool, must_include_names=floor, limit=40)
        context = clean_wiki_snippet(overview + at_a_glance)[:800]
    ...
    # merge_key_character_cast(pool, floor, llm_names)  # S3
"""
